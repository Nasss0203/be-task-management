...
import {
  PaginatedWorkspaceTemplateResponseDto,
  WorkspaceTemplateResponseDto,
} from '../dto/response/workspace-template.response.dto';
...
export class WorkspaceTemplatesServiceImpl implements WorkspaceTemplatesService {
  ...
  async findAll(
    where?: FindOptionsWhere<WorkspaceTemplateModel>,
  ...
  async findOneAvailableForUser(
    ...
    const template = await this.workspaceTemplateRepo.findOneAvailableForUser(
      id,
      userId,
    );
    if (!template) {
      throw new NotFoundException(
        `Workspace Template with ID ${id} not found or access denied`,
      );
    }
    return template;
  }
  ...
  async findAllAvailableForUser(
    ...
    const result = await this.workspaceTemplateRepo.findAllAvailableForUser(
      userId,
      filters,
    );

    const mappedData: WorkspaceTemplateResponseDto[] = result.data.map(
      (template) => {
