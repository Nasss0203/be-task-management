...
export class WorkspaceTemplatesController {
  ...
  @Public()
  findAll(@Query() dto: FindWorkspaceTemplatesDto, @Auth() auth?: IAuth) {
    return this.workspaceTemplatesApp.findAllAvailableForUser(auth?.id, dto);
  }

  @Get(':id')
  @ResponseMessage('Find template successfully')
  findOne(@Param('id') id: string, @Auth() auth?: IAuth) {
    return this.workspaceTemplatesApp.findOneAvailableForUser(
      id,
      auth?.id as any,
    );
  }
