...
describe('WorkspaceTemplatesController', () => {
  ...
  describe('findAll', () => {
    it('should call findAll with PUBLISHED and PUBLIC conditions', async () => {
      ...
      mockWorkspaceTemplatesApp.findAllAvailableForUser.mockResolvedValue(mockTemplates);

      const result = await controller.findAll({});

      expect(mockWorkspaceTemplatesApp.findAllAvailableForUser).toHaveBeenCalledWith(
        undefined,
        {}
      );
  ...
  describe('findOne', () => {
    it('should call findOne with correct id', async () => {
      ...
      mockWorkspaceTemplatesApp.findOneAvailableForUser.mockResolvedValue(mockTemplate);

      const result = await controller.findOne('1');

      expect(mockWorkspaceTemplatesApp.findOneAvailableForUser).toHaveBeenCalledWith('1', undefined);
