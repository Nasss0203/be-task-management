...
describe('UsageLimitEnforcerServiceImpl', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: BILLING_TYPES.repositories.UsageLimitRepository, useValue: mockRepo },
      ],
    }).compile();

    service = module.get<UsageLimitEnforcerServiceImpl>(UsageLimitEnforcerServiceImpl);
  });
  ...
  describe('checkProjectLimit', () => {
    it('should throw if limit not found', async () => {
      ...
      await expect(service.checkProjectLimit('ws-1')).rejects.toThrow(BadRequestException);
    });

    it('should pass if limit is unlimited', async () => {
      mockRepo.findByWorkspaceAndResource.mockResolvedValue({ limitValue: null });
  ...
  describe('syncProjectUsedValue', () => {
    ...
    it('should sync project used value', async () => {
      ...
      mockRepo.countProjectsByWorkspaceId.mockResolvedValue(3);
      await service.syncProjectUsedValue('ws-1');
      expect(mockRepo.save).toHaveBeenCalledWith({ ...usageLimit, usedValue: 3 }, undefined);
    });
  });
});
