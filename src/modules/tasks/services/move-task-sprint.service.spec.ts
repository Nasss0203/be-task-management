...
describe('MoveTaskSprintServiceImpl', () => {
  ...
  describe('move', () => {
    it('should call moveTaskToSprint on repository', async () => {
      ...
      ).toHaveBeenCalledWith('1', 'sprint-1', input.manager);
  ...
  describe('moveMany', () => {
    ...
    it('should throw BadRequestException if task workspace does not match', async () => {
      mockFindTaskRepository.findByIds.mockResolvedValue([
        { id: '1', workspaceId: 'ws-2', projectId: 'proj-1' },
      ]);
      await expect(
        service.moveMany({
          taskIds: ['1'],
    ...
    it('should throw BadRequestException if task project does not match', async () => {
      mockFindTaskRepository.findByIds.mockResolvedValue([
        { id: '1', workspaceId: 'ws-1', projectId: 'proj-2' },
      ]);
      await expect(
        service.moveMany({
          taskIds: ['1'],
    ...
    it('should call moveManyTaskToSprint on repository', async () => {
      ...
      mockFindTaskRepository.findByIds.mockResolvedValue([
        { id: '1', workspaceId: 'ws-1', projectId: 'proj-1' },
      ]);
      mockMoveTaskSprintRepository.moveManyTaskToSprint.mockResolvedValue(
        undefined,
      );
      ...
      expect(
        mockMoveTaskSprintRepository.moveManyTaskToSprint,
      ).toHaveBeenCalledWith(['1'], 'sprint-1', manager);
    });
  });
});
