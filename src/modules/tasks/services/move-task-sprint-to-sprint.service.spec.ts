...
describe('MoveTaskSprintToSprintServiceImpl', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<MoveTaskSprintToSprintServiceImpl>(MoveTaskSprintToSprintServiceImpl);
  });
  ...
  describe('move', () => {
    ...
    it('should throw BadRequestException if source and target sprint are the same', async () => {
      await expect(service.move({ ...input, targetSprintId: 'sprint-1' })).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if task workspace does not match', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-2' });
    ...
    it('should throw BadRequestException if task project does not match', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-2' });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if task is not in any sprint', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: null });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if task is not in source sprint', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-3' });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if source sprint not found', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
    ...
    it('should throw BadRequestException if source sprint workspace does not match', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce({ workspaceId: 'ws-2' });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if source sprint project does not match', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce({ workspaceId: 'ws-1', projectId: 'proj-2' });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if target sprint not found', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce({ workspaceId: 'ws-1', projectId: 'proj-1' });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce(null);
      await expect(service.move(input)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if source sprint is COMPLETED', async () => {
      mockFindTaskRepository.findOneTask.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.COMPLETED });
      mockFindSprintRepository.findOneSprint.mockResolvedValueOnce({ workspaceId: 'ws-1', projectId: 'proj-1' });
      await expect(service.move(input)).rejects.toThrow(BadRequestException);
    });
