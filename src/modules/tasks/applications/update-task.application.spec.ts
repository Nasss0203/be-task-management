...
describe('UpdateTaskApplicationImpl', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        UpdateTaskApplicationImpl,
        {
          ...
          useValue: mockUpdateTaskService,
        },
        {
  ...
  describe('updateTask', () => {
    ...
    it('should update task, log activity, and emit event inside transaction', async () => {
      ...
      expect(mockUpdateTaskService.updateTask).toHaveBeenCalledWith(
        input,
  ...
  describe('updateManyTasks', () => {
    ...
    it('should throw BadRequestException if workspaceId is missing', async () => {
      await expect(
        app.updateManyTasks({ ...input, workspaceId: '' }),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if projectId is missing', async () => {
      await expect(
        app.updateManyTasks({ ...input, projectId: '' }),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if task list is empty', async () => {
      await expect(
        app.updateManyTasks({ ...input, dto: { taskIds: [] } }),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if some tasks are missing', async () => {
      mockFindTaskService.findByIds.mockResolvedValue([
        { id: 'task-1', workspaceId: 'ws-1', projectId: 'proj-1' },
      ]);
      await expect(app.updateManyTasks(input)).rejects.toThrow(
        NotFoundException,
      );
    });
