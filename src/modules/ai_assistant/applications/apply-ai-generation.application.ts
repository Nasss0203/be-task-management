        ...
        if (generation.generationType === AiGenerationType.WORKSPACE_DRAFT) {
          ...
          if (!name) {
            throw new BadRequestException(
              'Workspace name is missing from draft',
            );
          }
          ...
          generation.generationType === AiGenerationType.PROJECT_DRAFT
        ) {
          const draft = outputData as unknown as ProjectDraftOutput & {
            tasks?: TaskDraftOutput[];
          };
          const name = draft.name;
          const key = draft.key;
                  ...
                  const task = await this.createTaskService.create(
                    {
                      ...
                      statusId,
                      priorityId,
                      estimateMinutes: estimatedHours
                        ? estimatedHours * 60
                        : null,
          ...
          if (!draft.tasks || !Array.isArray(draft.tasks)) {
            throw new BadRequestException(
              'Invalid task draft output structure',
            );
          }
          ...
          generation.generationType === AiGenerationType.WORKSPACE_TREE_DRAFT
        ) {
          ...
          await this.dataSource.transaction(
            async (transactionalEntityManager) => {
              for (const wsDraft of draft.workspaces) {
                const workspace =
                  await this.createWorkspaceTemplateService.create(
                    input.userId,
                    { name: wsDraft.name },
                    transactionalEntityManager,
                  );
                    ...
                    const task = await this.createTaskService.create(
                      {
                        ...
                        statusId,
                        priorityId,
                        estimateMinutes: taskDraft.estimatedHours
                          ? taskDraft.estimatedHours * 60
                          : null,
                        createdBy: input.userId,
                      },
                      transactionalEntityManager,
      ...
      await this.generationService.updateStatus({
        ...
        errorMessage:
          error instanceof Error
            ? error.message
            : 'Apply blocked due to quota or creation error',
      });
