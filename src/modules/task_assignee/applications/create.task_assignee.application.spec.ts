...
describe('CreateTaskAssigneeApplicationImpl', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: TASK_ASSIGNEE_TYPES.services.CreateTaskAssigneeService, useValue: mockCreateTaskAssigneeService },
        ...
        { provide: NOTIFICATION_TYPES.services.CreateNotificationService, useValue: mockCreateNotificationService },
        { provide: ACTIVITY_TYPES.services.CreateActivityService, useValue: mockCreateActivityService },
        { provide: EventEmitter2, useValue: mockEventEmitter },
      ],
    }).compile();

    application = module.get<CreateTaskAssigneeApplicationImpl>(CreateTaskAssigneeApplicationImpl);
  });
  ...
  describe('assign', () => {
    ...
    const task = { id: 'task-1', workspaceId: 'ws-1', projectId: 'proj-1', title: 'Task 1' };
    ...
    it('should throw NotFoundException if task not found', async () => {
      ...
      await expect(application.assign(input)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw ForbiddenException if actor is not a member of workspace', async () => {
      ...
      await expect(application.assign(input)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw BadRequestException if target user is not a member of workspace', async () => {
      ...
      mockFindMemberService.findMemberInWorkspace.mockResolvedValueOnce(null);
      await expect(application.assign(input)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should assign successfully and send notification when assigning other user', async () => {
      ...
      mockFindMemberService.findMemberInWorkspace.mockResolvedValueOnce(actorMember);
      mockFindMemberService.findMemberInWorkspace.mockResolvedValueOnce(targetMember);
      mockCreateTaskAssigneeService.assign.mockResolvedValue({ id: 'assignee-1', user: { id: 'user-2' } });
      mockCreateNotificationService.createNotification.mockResolvedValue({ id: 'noti-1' });

      const originalMapper = TaskAssigneeMapper.toResponse;
      TaskAssigneeMapper.toResponse = jest.fn().mockReturnValue({ isMapped: true });

      const result = await application.assign(input);

      expect(mockCreateTaskAssigneeService.assign).toHaveBeenCalledWith(input, undefined);
      expect(mockCreateActivityService.create).toHaveBeenCalledWith(expect.objectContaining({
        action: ActivityAction.TASK_ASSIGNED,
        field: 'assignee',
        newValue: 'user-2',
      }), undefined);
      expect(mockCreateNotificationService.createNotification).toHaveBeenCalled();
      expect(mockEventEmitter.emit).toHaveBeenCalledWith(REALTIME_EVENTS.NOTIFICATION_CREATED, expect.any(Object));
      expect(mockEventEmitter.emit).toHaveBeenCalledWith(REALTIME_EVENTS.TASK_UPDATED, expect.any(Object));
    ...
    it('should assign successfully without notification if self assign', async () => {
      const selfInput = { taskId: 'task-1', userId: 'user-1', assignedBy: 'user-1' };
      mockFindTaskService.findOneTask.mockResolvedValue(task);
      mockFindMemberService.findMemberInWorkspace.mockResolvedValue({ role_name: RoleName.MEMBER });
      mockCreateTaskAssigneeService.assign.mockResolvedValue({ id: 'assignee-1', user: { id: 'user-1' } });
      ...
      expect(mockCreateNotificationService.createNotification).not.toHaveBeenCalled();
