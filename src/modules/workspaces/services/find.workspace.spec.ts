...
describe('FindWorkspaceServiceImpl', () => {
  ...
  it('finds all workspaces by user id', async () => {
    ...
    findWorkspaceRepository.findWorkspacesByUserId.mockResolvedValue(
      mockWorkspaces,
    );

    const result = await service.findAllByUserId(userId);

    expect(findWorkspaceRepository.findWorkspacesByUserId).toHaveBeenCalledWith(
      userId,
    );
  ...
  it('finds one workspace by workspace id', async () => {
    ...
    findWorkspaceRepository.findOneWorkspaceById.mockResolvedValue(
      mockWorkspace,
    );

    const result = await service.findOneByWorkspaceId(userId, workspaceId);

    expect(findWorkspaceRepository.findOneWorkspaceById).toHaveBeenCalledWith(
      userId,
      workspaceId,
    );
  ...
  it('throws NOT_FOUND exception if workspace is not found by id', async () => {
    ...
    await expect(
      service.findOneByWorkspaceId(userId, workspaceId),
