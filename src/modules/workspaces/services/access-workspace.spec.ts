...
describe('AccessWorkspaceServiceImpl', () => {
  ...
  it('returns workspace access if user is a member', async () => {
    ...
    expect(accessWorkspaceRepository.findWorkspaceAccess).toHaveBeenCalledWith(userId, workspaceId);
  ...
  it('throws ForbiddenException if user is not a member', async () => {
    ...
    await expect(service.getWorkspaceAccess(userId, workspaceId)).rejects.toThrow(ForbiddenException);
    expect(accessWorkspaceRepository.findWorkspaceAccess).toHaveBeenCalledWith(userId, workspaceId);
  });
});
