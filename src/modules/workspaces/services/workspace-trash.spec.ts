...
describe('WorkspaceTrashServiceImpl', () => {
  ...
  describe('findDeletedWorkspacesByUserId', () => {
    it('returns deleted workspaces for user', async () => {
      ...
      workspaceTrashRepository.findDeletedWorkspacesByUserId.mockResolvedValue(mockWorkspaces);

      const result = await service.findDeletedWorkspacesByUserId(userId);

      expect(workspaceTrashRepository.findDeletedWorkspacesByUserId).toHaveBeenCalledWith(userId);
  ...
  describe('softDeleteWorkspace', () => {
    it('soft deletes a workspace successfully', async () => {
      ...
      workspaceTrashRepository.softDeleteWorkspace.mockResolvedValue(mockWorkspace);

      const result = await service.softDeleteWorkspace(userId, workspaceId);

      expect(workspaceTrashRepository.softDeleteWorkspace).toHaveBeenCalledWith(userId, workspaceId);
    ...
    it('throws NOT_FOUND exception if workspace to delete is not found', async () => {
      ...
      await expect(service.softDeleteWorkspace(userId, workspaceId)).rejects.toMatchObject({
  ...
  describe('restoreWorkspace', () => {
    it('restores a deleted workspace successfully', async () => {
      ...
      workspaceTrashRepository.restoreWorkspace.mockResolvedValue(mockWorkspace);

      const result = await service.restoreWorkspace(userId, workspaceId);

      expect(workspaceTrashRepository.restoreWorkspace).toHaveBeenCalledWith(userId, workspaceId);
      expect(result).toEqual(mockWorkspace);
    });
    ...
    it('throws NOT_FOUND exception if workspace to restore is not found', async () => {
      ...
      await expect(service.restoreWorkspace(userId, workspaceId)).rejects.toMatchObject({
        status: HttpStatus.NOT_FOUND,
      });
