...
describe('RbacSeedService', () => {
  ...
  const mockPermissionRepo = { findOne: jest.fn(), create: jest.fn(), save: jest.fn(), find: jest.fn() };
  const mockRoleRepo = { find: jest.fn(), create: jest.fn(), save: jest.fn() };
  const mockRolePermissionRepo = { find: jest.fn(), findOne: jest.fn(), create: jest.fn(), save: jest.fn(), delete: jest.fn() };
  ...
  it('should seed completely', async () => {
    ...
    mockPermissionRepo.find.mockResolvedValue([{ id: 'p1', code: 'workspace.read' }]);
    ...
    mockRoleRepo.save.mockImplementation(async (item) => ({ ...item, id: `r-${item.name}` }));
  ...
  it('should seed existing permission and existing role permission', async () => {
    ...
    mockPermissionRepo.find.mockResolvedValue([{ id: 'p1', code: 'workspace.read' }]);
    mockWorkspaceRepo.find.mockResolvedValue([{ id: 'ws1' }]);
    mockRoleRepo.find.mockResolvedValue([{ id: 'r1', name: 'ADMIN' }, { id: 'r2', name: 'MEMBER' }, { id: 'r3', name: 'GUEST' }, { id: 'r4', name: 'SYSTEM_ADMIN' }, { id: 'r5', name: 'SUPER_ADMIN' }, { id: 'r6', name: 'OWNER' }, { id: 'r7', name: 'VIEWER' }]);
