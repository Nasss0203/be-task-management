...
describe('GetSprintDetailApplicationImpl', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: SPRINT_TYPES.services.GetSprintDetailService, useValue: mockGetSprintDetailService },
      ],
    }).compile();

    app = module.get<GetSprintDetailApplicationImpl>(GetSprintDetailApplicationImpl);
  });
  ...
  describe('getSprintDetail', () => {
    it('should return mapped sprint detail', async () => {
      mockGetSprintDetailService.getSprintDetail.mockResolvedValue({ id: 'sprint-1' });
      const result = await app.getSprintDetail({ workspaceId: 'ws-1', projectId: 'proj-1', sprintId: 'sprint-1' });
