...
describe('UpdateSprintServiceImpl', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: SPRINT_TYPES.repositories.UpdateSprintRepository, useValue: mockUpdateSprintRepository },
      ],
    }).compile();
  ...
  describe('updateSprint', () => {
    it('should throw NotFoundException if sprint not found', async () => {
      ...
      await expect(service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1' } as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if workspace mismatch', async () => {
      ...
      await expect(service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1' } as any)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if sprint name is empty string', async () => {
      ...
      await expect(service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1', name: '   ' } as any)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if updating locked field of active sprint', async () => {
      ...
      await expect(service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1', name: 'New Name' } as any)).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException if startAt >= endAt', async () => {
      mockFindSprintRepository.findOneSprint.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.PLANNED });
      await expect(
        service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1', startAt: new Date('2023-01-02'), endAt: new Date('2023-01-01') } as any)
    ...
    it('should throw NotFoundException if update fails', async () => {
      ...
      await expect(service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1', name: 'Valid' } as any)).rejects.toThrow(NotFoundException);
    });

    it('should update sprint', async () => {
      mockFindSprintRepository.findOneSprint.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.PLANNED });
      mockUpdateSprintRepository.updateSprint.mockResolvedValue({ id: '1' });
      const result = await service.updateSprint({ id: '1', workspaceId: 'ws-1', projectId: 'proj-1', name: 'Valid' } as any);
