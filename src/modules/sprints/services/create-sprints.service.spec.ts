...
describe('CreateSprintServiceImpl', () => {
  ...
  const mockFindSprintRepository = {
    getNextDefaultSprintName: jest.fn(),
    existsByProjectIdAndName: jest.fn(),
  };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CreateSprintServiceImpl,
        {
          ...
          useValue: mockCreateSprintRepository,
        },
        {
  ...
  describe('create', () => {
    it('should throw NotFoundException if project not found', async () => {
      ...
      await expect(
        service.create({ workspaceId: 'ws-1', projectId: 'proj-1' } as any),
    ...
    it('should throw BadRequestException if startAt >= endAt', async () => {
      ...
      mockFindSprintRepository.existsByProjectIdAndName.mockResolvedValue(
        false,
      );
      await expect(
        service.create({
    ...
    it('should create sprint with default name if name is empty', async () => {
      ...
      mockFindSprintRepository.getNextDefaultSprintName.mockResolvedValue(
        'Sprint 1',
      );
