...
describe('StartSprintServiceImpl', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: SPRINT_TYPES.repositories.StartSprintRepository, useValue: mockStartSprintRepository },
  ...
  describe('startSprint', () => {
    it('should throw NotFoundException if sprint not found', async () => {
      ...
      await expect(service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1' } as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if workspace mismatch', async () => {
      ...
      await expect(service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1' } as any)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if startAt >= endAt', async () => {
      ...
      await expect(
        service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1', startAt: new Date('2023-01-02'), endAt: new Date('2023-01-01') } as any)
    ...
    it('should throw BadRequestException if name is empty string', async () => {
      mockFindSprintRepository.findOneSprint.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.PLANNED });
      await expect(
        service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1', name: '' } as any)
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException if startAt is invalid date', async () => {
      mockFindSprintRepository.findOneSprint.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.PLANNED });
      await expect(
        service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1', startAt: new Date('invalid') } as any)
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if startSprint fails', async () => {
      mockFindSprintRepository.findOneSprint.mockResolvedValue({ workspaceId: 'ws-1', projectId: 'proj-1', status: SprintStatus.PLANNED, startAt: new Date(), endAt: new Date(Date.now() + 10000) });
    ...
    it('should start sprint', async () => {
      ...
      const result = await service.startSprint({ sprintId: '1', workspaceId: 'ws-1', projectId: 'proj-1' } as any);
