...
describe('CreateWorkspaceInviteLinkApplicationImpl', () => {
  ...
  beforeEach(async () => {
    ...
    app = module.get<CreateWorkspaceInviteLinkApplicationImpl>(CreateWorkspaceInviteLinkApplicationImpl);
  });
  ...
  it('should create link', async () => {
    ...
    const result = await app.createLink('ws-1', 'u-1', { role_name: RoleName.MEMBER } as any);
    expect(mockRepo.save).toHaveBeenCalled();
    expect(result.id).toEqual('inv-1');
  });

  it('should throw if workspaceId missing', async () => {
    await expect(app.createLink('', 'u-1', { role_name: RoleName.MEMBER } as any)).rejects.toThrow(BadRequestException);
  });
  ...
  it('should throw if role is OWNER and inviter is not OWNER', async () => {
    ...
    await expect(app.createLink('ws-1', 'u-1', { role_name: RoleName.OWNER } as any)).rejects.toThrow(ForbiddenException);
  });
});
