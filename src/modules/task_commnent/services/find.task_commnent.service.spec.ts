...
describe('FindTaskCommentServiceImpl', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<FindTaskCommentServiceImpl>(
      FindTaskCommentServiceImpl,
    );
  });
  ...
  describe('findByTaskId', () => {
    ...
    it('should throw NotFoundException if task not found', async () => {
      ...
      await expect(service.findByTaskId(input)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should return comments successfully', async () => {
      ...
      mockFindTaskCommentReposiroty.findByTaskId.mockResolvedValue([
        { id: 'comment-1' },
      ]);

      const result = await service.findByTaskId(input);

      expect(mockFindTaskCommentReposiroty.findByTaskId).toHaveBeenCalledWith(
        'ws-1',
        'proj-1',
        'task-1',
      );
