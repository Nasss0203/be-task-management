...
describe('DeleteBoardApplicationImpl', () => {
  ...
  const mockUnitOfWork = {
    runInTransaction: jest.fn(
      async (fn: (manager: unknown) => Promise<unknown>) => fn(mockManager),
    ),
  };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: WORKSPACE_TYPES.uow.UnitOfWork, useValue: mockUnitOfWork },
        {
          ...
          useValue: mockFindBoardService,
        },
        {
          provide: BOARD_TYPES.services.DeleteBoardService,
          useValue: mockDeleteBoardService,
        },
        {
  ...
  describe('delete', () => {
    it('should delete board', async () => {
      mockFindBoardService.findOneBoardForRestore.mockResolvedValue({
        id: 'b-1',
      });
  ...
  describe('restore', () => {
    it('should restore board', async () => {
      ...
      expect(mockDeleteBoardService.restoreBoard).toHaveBeenCalledWith({
        boardId: 'b-1',
      });
    });
