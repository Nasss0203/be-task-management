...
import { UserActivity, UserActivityType } from '../domain/entities/user_activity.entity';
...
describe('UserActivityService', () => {
  ...
  it('should record user activity', async () => {
    ...
    expect(mockRepo.insert).toHaveBeenCalledWith({ userId: 'u-1', type: UserActivityType.OPEN_APP });
  });
  ...
  it('should record login', async () => {
    ...
    expect(mockRepo.insert).toHaveBeenCalledWith({ userId: 'u-1', type: UserActivityType.LOGIN });
  });
  ...
  it('should record open workspace', async () => {
    ...
    expect(mockRepo.insert).toHaveBeenCalledWith({ userId: 'u-1', type: UserActivityType.OPEN_WORKSPACE });
  });
});
