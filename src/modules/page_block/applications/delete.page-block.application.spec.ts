...
describe('DeletePageBlockApplicationImpl', () => {
  ...
  beforeEach(async () => {
    ...
    app = module.get<DeletePageBlockApplicationImpl>(
      DeletePageBlockApplicationImpl,
    );
  });
  ...
  describe('delete', () => {
    it('should delete page block', async () => {
      mockFindService.findOnePageBlockForRestore.mockResolvedValue({
        id: 'pb-1',
        pageId: 'page-1',
      });
      await app.delete({ workspaceId: 'ws-1', blockId: 'pb-1', userId: 'u-1' });

      expect(mockDeleteService.softDeletePageBlock).toHaveBeenCalledWith({
        blockId: 'pb-1',
        deletedBy: 'u-1',
      });
    ...
    it('should throw if not found', async () => {
      ...
      await expect(
        app.delete({ workspaceId: 'ws-1', blockId: 'pb-1', userId: 'u-1' }),
  ...
  describe('restore', () => {
    it('should restore page block', async () => {
      ...
      expect(mockDeleteService.restorePageBlock).toHaveBeenCalledWith({
        blockId: 'pb-1',
      });
    });
    ...
    it('should throw if not deleted', async () => {
      ...
      await expect(
        app.restore({ workspaceId: 'ws-1', blockId: 'pb-1', userId: 'u-1' }),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
