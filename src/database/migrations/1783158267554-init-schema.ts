...
export class InitSchema1783158267554 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."ai_generation_type_enum" AS ENUM('WORKSPACE_DRAFT', 'PROJECT_DRAFT', 'TASK_DRAFT', 'SPRINT_PLAN', 'SPRINT_SUMMARY', 'DASHBOARD_INSIGHT')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."ai_provider_enum" AS ENUM('GEMINI', 'OPENAI', 'DEEPSEEK')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."ai_generation_status_enum" AS ENUM('PROCESSING', 'GENERATED', 'APPLIED', 'DISCARDED', 'FAILED')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."ai_message_role_enum" AS ENUM('USER', 'ASSISTANT', 'SYSTEM', 'TOOL')`,
    );
    await queryRunner.query(
      `CREATE TABLE "ai_messages" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "conversation_id" uuid NOT NULL, "role" "public"."ai_message_role_enum" NOT NULL, "content" text NOT NULL, "context" jsonb, "metadata" jsonb, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_a390434d4a515ba18a41bc996c2" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
